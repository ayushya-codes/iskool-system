...
export default function Coursework() {
  ...
  const cards = [
    { label: 'Assignments', value: activeTab === 'assignments' ? data.length : '—', icon: ClipboardList, color: '[color-mix(in_srgb,var(--accent-secondary)_10%,transparent)]0' },
    { label: 'Syllabus Logs', value: activeTab === 'syllabus' ? data.length : '—', icon: FileText, color: '[color-mix(in_srgb,var(--success)_10%,transparent)]0' },
    { label: 'Rooms', value: activeTab === 'rooms' ? data.length : '—', icon: BookOpen, color: 'gradient-bg' },
    { label: 'Timetables', value: activeTab === 'timetables' ? data.length : '—', icon: Calendar, color: '[color-mix(in_srgb,var(--info)_10%,transparent)]0' },
  ];
    ...
    return (
      <div>
        ...
        <div className="rounded-xl theme-card p-6">
          <p className="text-sm theme-text-muted">Coursework information for your child is available on the <strong>My Child</strong> page.</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        {activeTab !== 'rooms' && (
          <div className="flex items-center gap-2">
            <select value={selectedClass} onChange={(e) => { setSelectedClass(e.target.value); setSelectedDivision(''); }} className="rounded-lg theme-input px-3 py-2 text-sm focus-ring">
              <option value="">Select Class</option>
              {classes.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <select value={selectedDivision} onChange={(e) => setSelectedDivision(e.target.value)} disabled={!selectedClass} className="rounded-lg theme-input px-3 py-2 text-sm focus-ring disabled:[var(--sidebar-hover-bg)]">
              <option value="">Select Division</option>
              {divisions.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
            </select>
            {activeTab === 'syllabus' && (
              <select value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)} disabled={!selectedDivision} className="rounded-lg theme-input px-3 py-2 text-sm focus-ring disabled:[var(--sidebar-hover-bg)]">
        ...
        {canManage && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleOpenCreate} disabled={activeTab !== 'rooms' && !selectedDivision} className="inline-flex items-center gap-2 rounded-lg gradient-bg px-4 py-2 text-sm font-semibold text-white gradient-bg-hover transition-colors disabled:opacity-50">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={`Add ${TABS.find((t) => t.key === activeTab)?.label.slice(0, -1)}`}>
        <form onSubmit={handleSave} className="space-y-4">
          {activeTab === 'assignments' && (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Due Date"><input type="date" required value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              </div>
              <FormField label="Subject">
                {subjects.length === 0 ? (
                  <p className="text-sm [color:var(--danger)] [color-mix(in_srgb,var(--danger)_10%,transparent)] rounded-lg px-3 py-2">No subjects available. Please create subjects first in the Academic section.</p>
                ) : (
                  <select required value={form.subjectId} onChange={(e) => setForm({ ...form, subjectId: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring">
                    <option value="">Select Subject</option>
          ...
          {activeTab === 'syllabus' && (
            <>
              <FormField label="Content / Topic Covered"><input type="text" required value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              <FormField label="Date"><input type="date" required value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              <FormField label="Subject">
                {subjects.length === 0 ? (
                  <p className="text-sm [color:var(--danger)] [color-mix(in_srgb,var(--danger)_10%,transparent)] rounded-lg px-3 py-2">No subjects available. Please create subjects first in the Academic section.</p>
                ) : (
          ...
          {activeTab === 'rooms' && (
            <>
              <FormField label="Room Name"><input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Capacity"><input type="number" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
                <FormField label="Floor"><input type="text" value={form.floor} onChange={(e) => setForm({ ...form, floor: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              </div>
              <FormField label="Building"><input type="text" value={form.building} onChange={(e) => setForm({ ...form, building: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
            </>
          )}
          {activeTab === 'timetables' && (
            <>
              <FormField label="Batch ID"><input type="number" required value={form.batchId} onChange={(e) => setForm({ ...form, batchId: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
              <FormField label="Effective Date"><input type="date" required value={form.effectiveDate} onChange={(e) => setForm({ ...form, effectiveDate: e.target.value })} className="w-full rounded-lg theme-input px-3 py-2 text-sm focus-ring" /></FormField>
            </>
          )}
